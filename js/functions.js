...
Game.do = (function() {
    ...
    for (var y = 0; y <= Game.config.boardHeight; y++) {
      for (var x = 0; x <= Game.config.boardLength; x++) {
        if (Game.check.isPositionTaken(x, y)) {
          ...
          cell.firstElementChild.classList.add((Game.board[y][x] === 1) ? "black" : "red");
        }
      }
    }
  }
  ...
  function httpGet(theUrl,board)
  {
      var xmlHttp = new XMLHttpRequest();
      console.log(board)
      xmlHttp.open( "GET", theUrl+"?board="+JSON.stringify(board)+"&player="+Game.currentPlayer, false ); // false for synchronous request
      xmlHttp.send( null );
